...
def To2(a): # 10진수를 2진수로 바꾸는 함수
    res = []
    while a:
        res.append(a % 2) #마지막 자리부터 2진수로 값을 구함, 따라서 1의 자리수 부터 2, 4의 자리로 거꾸로 나오게 됨
...
def base64(k): # 코드를 문자로 변경
    ...
    if temp >= A and temp < a: # 만약 temp에 해당하는 값이 대문자라면 (base64 기준)
        ...
        return chr(ord('a') + temp - a) # 아스키 코드 기준 소문자이므로, 'a'에서 temp - a(temp는 26+a 임 따라서 -a를 한 후 a에 추가)를 더해줌
...
def toBaseCode(k): # base64 문자를 10진수로 변환 
    ...
    if temp >= ord('A') and temp <= ord('Z'): # 대문자라면
        ...
        return temp - ord('a') + a #base64에 해당하는 소문자 코드로 변환(이 때 소문자는 a 부터 시작이므로 a를 더해줌)
